...
public class PedidoServiceImpl implements IPedidoService {
    ...
    public Pedido crearDesdeCarrito(Carrito carrito, CheckoutDTO dto, Cliente cliente) {
        ...
        double subtotalItems       = carrito.getTotal();           // suma de precios ya descontados por ítem
        double ahorroProductos     = carrito.getAhorroProductos(); // ahorro por descuentos automáticos
        double descuentoCupon      = carrito.getMontoDescuentoCupon() != null
                                        ? carrito.getMontoDescuentoCupon() : 0.0;
        String cuponAplicado       = carrito.getCuponAplicado();

        double subtotal = Math.max(0, subtotalItems - descuentoCupon); // neto final antes de IVA
        ...
        Pedido pedido = Pedido.builder()
                ...
                .iva(iva)
                .total(total)
                .montoAhorroProductos(ahorroProductos > 0 ? ahorroProductos : null)
                .montoDescuentoCupon(descuentoCupon > 0 ? descuentoCupon : null)
                .cuponAplicado(cuponAplicado)
                .estado(EstadoPedido.PENDIENTE)
                .build();

        // 5. Crear snapshots de ítems (incluye precio original si hubo descuento)
        for (ItemCarrito item : carrito.getItems()) {
            ItemPedido itemPedido = ItemPedido.builder()
                    ...
                    .precioOriginal(item.getPrecioOriginal())
                    .build();
        ...
        // 5b. Incrementar el contador de usos del cupón si se usó uno
        if (cuponAplicado != null) {
            cuponRepo.findByCodigoIgnoreCase(cuponAplicado).ifPresent(c -> {
    ...
    public List<Pedido> listarTodos(String filtroEstado, LocalDate fechaDesde, LocalDate fechaHasta,
                                     String emailBuscar, String orden) {
        ...
        boolean filtraEmail  = emailBuscar != null && !emailBuscar.isBlank();
        ...
        if (filtraEmail) {
            // Con búsqueda por email
            if (filtraTodos) {
                resultado = filtraFechas
                        ? pedidoRepo.findByEmailAndFechaBetween(emailBuscar, desde, hasta)
                        : pedidoRepo.findByEmailContainingIgnoreCase(emailBuscar);
            } else {
                try {
                    EstadoPedido estado = EstadoPedido.valueOf(filtroEstado);
                    resultado = filtraFechas
                            ? pedidoRepo.findByEstadoAndEmailAndFechaBetween(estado, emailBuscar, desde, hasta)
                            : pedidoRepo.findByEstadoAndEmailContainingIgnoreCase(estado, emailBuscar);
        ...
        // Ordenar por columna si se solicitó
        if (orden != null) {
            switch (orden) {
                case "id_asc"      -> resultado.sort(java.util.Comparator.comparingLong(Pedido::getId));
                case "id_desc"     -> resultado.sort(java.util.Comparator.comparingLong(Pedido::getId).reversed());
                case "cliente_asc" -> resultado.sort(java.util.Comparator.comparing(p -> p.getNombreParaMostrar().toLowerCase()));
                case "cliente_desc"-> resultado.sort(java.util.Comparator.comparing((Pedido p) -> p.getNombreParaMostrar().toLowerCase()).reversed());
                case "email_asc"   -> resultado.sort(java.util.Comparator.comparing(p -> p.getEmailContacto().toLowerCase()));
                case "email_desc"  -> resultado.sort(java.util.Comparator.comparing((Pedido p) -> p.getEmailContacto().toLowerCase()).reversed());
                case "items_asc"   -> resultado.sort(java.util.Comparator.comparingInt(p -> p.getItems().size()));
                case "items_desc"  -> resultado.sort(java.util.Comparator.comparingInt((Pedido p) -> p.getItems().size()).reversed());
                case "monto_asc"   -> resultado.sort(java.util.Comparator.comparingDouble(Pedido::getTotal));
                case "monto_desc"  -> resultado.sort(java.util.Comparator.comparingDouble(Pedido::getTotal).reversed());
                case "fecha_asc"   -> resultado.sort(java.util.Comparator.comparing(Pedido::getFechaPedido));
                case "fecha_desc"  -> resultado.sort(java.util.Comparator.comparing(Pedido::getFechaPedido).reversed());
                case "estado_asc"  -> resultado.sort(java.util.Comparator.comparing(p -> p.getEstado().name()));
                case "estado_desc" -> resultado.sort(java.util.Comparator.comparing((Pedido p) -> p.getEstado().name()).reversed());
            }
        }
