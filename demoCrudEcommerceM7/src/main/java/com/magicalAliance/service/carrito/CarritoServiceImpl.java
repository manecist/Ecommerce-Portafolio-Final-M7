...
public class CarritoServiceImpl implements ICarritoService {
    ...
    public void agregarProducto(Carrito carrito, Long productoId, int cantidad) {
        ...
        // Calcular precio con descuento automático (producto > subcategoría > categoría > global)
        double precioFinal = descuentoService.calcularPrecioConDescuento(producto);
        double precioOriginal = producto.getPrecio();
        boolean tieneDescuento = precioFinal < precioOriginal;
        ...
        if (itemExistente.isPresent()) {
            ...
            // Actualizar precio si cambió el descuento
            item.setPrecioUnitario(precioFinal);
            item.setPrecioOriginal(tieneDescuento ? precioOriginal : null);
            itemRepo.save(item);
        } else {
        ...
        recalcularCuponSiAplica(carrito);
    }
    ...
    public void actualizarCantidad(Long itemId, int nuevaCantidad) {
        ...
        item.setCantidad(nuevaCantidad);
        itemRepo.save(item);
        recalcularCuponSiAplica(item.getCarrito());
    }
    ...
    public void aplicarCupon(Carrito carrito, String codigo) {
        descuentoService.aplicarCupon(carrito, codigo);
    }

    @Override
    public void quitarCupon(Carrito carrito) {
        descuentoService.quitarCupon(carrito);
    }
    ...
    /**
     * Si hay un cupón aplicado al carrito, recalcula su monto según el nuevo total.
     * Si el cupón ya no es válido (agotado, expirado, no llega al mínimo), lo quita.
     */
    private void recalcularCuponSiAplica(Carrito carrito) {
        if (carrito.getCuponAplicado() == null) return;
        cuponRepo.findByCodigoIgnoreCase(carrito.getCuponAplicado()).ifPresentOrElse(c -> {
            double total = carrito.getTotal();
            if (c.estaDisponible() && total >= c.getMontoMinimo()) {
                carrito.setMontoDescuentoCupon(c.calcularDescuento(total));
            } else {
                carrito.setCuponAplicado(null);
                carrito.setMontoDescuentoCupon(null);
            }
