...
public class PedidoController {
    ...
    public String gestionPedidos(
            ...
            @RequestParam(required = false) String emailBuscar,
            @RequestParam(required = false) String orden,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            Model model) {

        List<Pedido> todos = pedidoService.listarTodos(filtroEstado, fechaDesde, fechaHasta, emailBuscar, orden);
        model.addAttribute("pedidos", PaginacionHelper.paginar(todos, page, size, model));
        model.addAttribute("filtroActual", filtroEstado);
        model.addAttribute("estados", EstadoPedido.values());
        model.addAttribute("fechaDesde", fechaDesde);
        model.addAttribute("fechaHasta", fechaHasta);
        model.addAttribute("emailBuscar", emailBuscar);
